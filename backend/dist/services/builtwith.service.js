...
class BuiltWithService {
    constructor() {
        this.baseUrl = "https://api.builtwith.com/free1/api.json";
        this.apiKey = process.env.BUILTWITH_API_KEY || "";
        if (!this.apiKey) {
            console.warn("⚠️  BuiltWith API key not provided. Using mock data for development.");
        }
    }
    async analyzeTechnologies(url) {
        try {
            if (!this.apiKey) {
                return this.getMockTechnologies(url);
            }
            const response = await fetch(`${this.baseUrl}?KEY=${this.apiKey}&LOOKUP=${encodeURIComponent(url)}`);
            if (!response.ok) {
            ...
            throw new Error(`Failed to analyze website technologies: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
...
exports.default = BuiltWithService;
//# sourceMappingURL=builtwith.service.js.map